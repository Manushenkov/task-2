function prepareData(rawData, sprintObj) {
  ...
  // добавление "actuve": true
	...
	// дополнтельная сортировка по summary, для коммитов с одним и тем же timestamp
	currentSprintCommits.sort((a, b) => {
	...
	*/
	...
	const diagram = {
	  ...
	  "data": {
	    ...
	    "categories": [
	      {
	      	"title": "> 1001 строки",
	       	"valueText": `${sprints[currentSprintIndex].hugeCommits} коммитов`,
	        	"differenceText": `${getSign(sprints[currentSprintIndex].hugeCommits - sprints[previousSprintIndex].hugeCommits)} коммитов`
			},
	      {
	      	"title": "501 — 1000 строк", 
	      	"valueText": `${sprints[currentSprintIndex].bigCommits} коммитов`,
	      	"differenceText": `${getSign(sprints[currentSprintIndex].bigCommits - sprints[previousSprintIndex].bigCommits)} коммитов`
	      },
	      {
	      	"title": "101 — 500 строк",
	      	"valueText": `${sprints[currentSprintIndex].smallCommits} коммитов`,
	      	"differenceText": `${getSign(sprints[currentSprintIndex].smallCommits - sprints[previousSprintIndex].smallCommits)} коммитов`},
	      {
	      	"title": "1 — 100 строк",
	      	"valueText": `${sprints[currentSprintIndex].tinyCommits} коммитов`,
	      	"differenceText": `${getSign(sprints[currentSprintIndex].tinyCommits - sprints[previousSprintIndex].tinyCommits)} коммитов`}
	    ]
		}}
	...
	const sprintStartDay = new Date(sprints[currentSprintIndex].startAt).getDate();
	const sprintFinishDay = new Date(sprints[currentSprintIndex].finishAt).getDate();

	currentSprintCommitsWithEmptyCommits.forEach(commit => {
		let time = new Date(commit.timestamp)
		activityStat[time.getDate() - sprintStartDay][time.getHours()] += 1 
	})
